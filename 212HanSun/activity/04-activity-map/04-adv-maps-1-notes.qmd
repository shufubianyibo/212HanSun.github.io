---
title: "Advanced spatial visualization (Notes)"
...
## Additional resources {.unnumbered .smaller}
...
## Setup {.unnumbered .smaller}

Create the following directory structure for this activity:
...
## Spatial Data in R {.unnumbered .smaller}

See [Spatial data notes](https://bcheggeseth.github.io/212_spring_2025/notes/spatial-data.html) for basics of CRS and spatial data types.


### Download Shapefiles {.unnumbered .smaller}
...
### Read in Files {.unnumbered .smaller}
...
mn_cities <- st_read("shp_loc_pop_centers/city_and_township_population_centers.shp")
mn_water <- st_read("shp_water_lakes_rivers/LakesAndRivers.shp")
```
...
## MN map with multiple layers {.unnumbered .smaller}
...
### Get county boundaries {.unnumbered .smaller}
...
### Unifying CRSs across different spatial datasets {.unnumbered .smaller}
...
### Counties + cities {.unnumbered .smaller}
...
### Customize Colors {.unnumbered .smaller}
...
The boundaries of the cities are clearer when large cities have overlaps with each other.

- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?

It changes the appearance of the color scale under the map.
...
### Adding elevation raster data {.unnumbered .smaller}
...
### Zoom in to Twin Cities and add water {.unnumbered .smaller}
...
Let's add to the above code chunk to save the map above to an image file called `tc_map_zoom.png` in the `results` folder. The code example below shows a general template for saving a plot to file. Choose a reasonable width and height. (There are also `jpeg()` and `pdf()` functions for writing images.)
...
## Going Beyond - Twin Cities map with `leaflet` {.unnumbered .smaller}
